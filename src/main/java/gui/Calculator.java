...
public class Calculator extends VBox implements EventHandler<ActionEvent>{
	...
	public void handle(ActionEvent event) {
		...
		String pantalla = this.displayText.getText();
		if (pantalla.equals("ERROR") || pantalla.contains("=")) {
			resultado = 0;
			number1 = "";
			number2 = "";
			operator = null;
		...
		this.displayText.setText(pantalla+valor);
		switch(valor) {
		case "+": 
			operator = "+";
			break;
		...
		case "=":
			try {
				...
				this.displayText.setText(pantalla+valor+resultado);
			}
